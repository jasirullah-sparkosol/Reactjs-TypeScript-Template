...
export default function TablePagination({ getPageCount, setPageIndex, setPageSize, getState, initialPageSize }: TablePaginationProps) {
    ...
    // const [limit, setLimit] = useState(getState().pagination.pageIndex + 1);
    if (initialPageSize) {
                ...
                <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    <TextField
                        ...
                        onChange={({ target: { value } }) => {
                            if (value === '') {
                                setLimit('' as any); // Allow empty input
                                setPageIndex(0); // Optionally reset pageIndex when input is cleared
                            } else {
                                const numericValue = Number(value);
                                if (numericValue >= 0) {
                                    setLimit(numericValue);
                                    setPageIndex(numericValue - 1);
                                }
                            }
                        }}
                        sx={{
